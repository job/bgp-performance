...
class OpenBGP89(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp89'):
        super(OpenBGP89, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp89', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('8.9')
        super(OpenBGP89, cls).build_image(force, tag, nocache)


class OpenBGPTarget89(OpenBGP89, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp89_target'
    CONFIG_FILE_NAME = 'bgpd.conf'

    def __init__(self, host_dir, conf, image='bgperf/openbgp89'):
        super(OpenBGPTarget89, self).__init__(host_dir, conf, image=image)
