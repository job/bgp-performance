...
class OpenBGP76(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp76'):
        super(OpenBGP76, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp76', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('7.6')
        super(OpenBGP76, cls).build_image(force, tag, nocache)


class OpenBGPTarget76(OpenBGP76, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp76_target'
    CONFIG_FILE_NAME = 'bgpd.conf'

    def __init__(self, host_dir, conf, image='bgperf/openbgp76'):
        super(OpenBGPTarget76, self).__init__(host_dir, conf, image=image)
