...
class OpenBGP83(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp83'):
        super(OpenBGP83, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp83', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('8.3')
        super(OpenBGP83, cls).build_image(force, tag, nocache)


class OpenBGPTarget83(OpenBGP83, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp83_target'
    CONFIG_FILE_NAME = 'bgpd.conf'

    def __init__(self, host_dir, conf, image='bgperf/openbgp83'):
        super(OpenBGPTarget83, self).__init__(host_dir, conf, image=image)
