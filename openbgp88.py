...
class OpenBGP88(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp88'):
        super(OpenBGP88, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp88', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('8.8')
        super(OpenBGP88, cls).build_image(force, tag, nocache)


class OpenBGPTarget88(OpenBGP88, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp88_target'
    CONFIG_FILE_NAME = 'bgpd.conf'

    def __init__(self, host_dir, conf, image='bgperf/openbgp88'):
        super(OpenBGPTarget88, self).__init__(host_dir, conf, image=image)
