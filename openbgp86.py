...
class OpenBGP86(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp86'):
        super(OpenBGP86, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp86', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('8.6')
        super(OpenBGP86, cls).build_image(force, tag, nocache)


class OpenBGPTarget86(OpenBGP86, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp86_target'
    CONFIG_FILE_NAME = 'bgpd.conf'

    def __init__(self, host_dir, conf, image='bgperf/openbgp86'):
        super(OpenBGPTarget86, self).__init__(host_dir, conf, image=image)
