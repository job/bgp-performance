...
class OpenBGP77(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp77'):
        super(OpenBGP77, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp77', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('7.7')
        super(OpenBGP77, cls).build_image(force, tag, nocache)


class OpenBGPTarget77(OpenBGP77, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp77_target'
    CONFIG_FILE_NAME = 'bgpd.conf'

    def __init__(self, host_dir, conf, image='bgperf/openbgp77'):
        super(OpenBGPTarget77, self).__init__(host_dir, conf, image=image)
