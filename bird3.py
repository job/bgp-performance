...
class BIRD3(Container):

    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/bird3', name=None):
        super(BIRD3, self).__init__(name if name is not None else self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/bird3', checkout='HEAD', branch='master', nocache=False):
        cls.dockerfile = '''
FROM ubuntu:latest
WORKDIR /root
...
RUN cd bird && git checkout {} && autoreconf && ./configure && make && make install
'''.format('v3.1.4')
        super(BIRD3, cls).build_image(force, tag, nocache)


class BIRD3Target(BIRD3, Target):

    CONTAINER_NAME = 'bgperf_bird3_target'
