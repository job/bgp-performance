...
class OpenBGP73(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp73'):
        super(OpenBGP73, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp73', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('7.3')
        super(OpenBGP73, cls).build_image(force, tag, nocache)


class OpenBGPTarget73(OpenBGP73, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp73_target'
    CONFIG_FILE_NAME = 'bgpd.conf'

    def __init__(self, host_dir, conf, image='bgperf/openbgp73'):
        super(OpenBGPTarget73, self).__init__(host_dir, conf, image=image)
