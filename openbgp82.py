...
class OpenBGP82(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp82'):
        super(OpenBGP82, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp82', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('8.2')
        super(OpenBGP82, cls).build_image(force, tag, nocache)


class OpenBGPTarget82(OpenBGP82, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp82_target'
    CONFIG_FILE_NAME = 'bgpd.conf'

    def __init__(self, host_dir, conf, image='bgperf/openbgp82'):
        super(OpenBGPTarget82, self).__init__(host_dir, conf, image=image)
