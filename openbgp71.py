...
class OpenBGP71(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp71'):
        super(OpenBGP71, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp71', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('7.1')
        super(OpenBGP71, cls).build_image(force, tag, nocache)


class OpenBGPTarget71(OpenBGP71, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp71_target'
