...
class OpenBGP72(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp72'):
        super(OpenBGP72, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp72', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('7.2')
        super(OpenBGP72, cls).build_image(force, tag, nocache)


class OpenBGPTarget72(OpenBGP72, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp72_target'
