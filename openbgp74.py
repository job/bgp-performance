...
class OpenBGP74(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp74'):
        super(OpenBGP74, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp74', checkout='', nocache=False):
...
RUN cd /root/openbgpd-{0} && \
    ...
    make install
'''.format('7.4')
        super(OpenBGP74, cls).build_image(force, tag, nocache)


class OpenBGPTarget74(OpenBGP74, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp74_target'
    CONFIG_FILE_NAME = 'bgpd.conf'

    def __init__(self, host_dir, conf, image='bgperf/openbgp74'):
        super(OpenBGPTarget74, self).__init__(host_dir, conf, image=image)
