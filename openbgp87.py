...
class OpenBGP87(Container):
    CONTAINER_NAME = None
    GUEST_DIR = '/root/config'

    def __init__(self, host_dir, conf, image='bgperf/openbgp87'):
        super(OpenBGP87, self).__init__(self.CONTAINER_NAME, image, host_dir, self.GUEST_DIR, conf)

    @classmethod
    def build_image(cls, force=False, tag='bgperf/openbgp87', checkout='', nocache=False):
...
class OpenBGPTarget87(OpenBGP87, Target):
    
    CONTAINER_NAME = 'bgperf_openbgp87_target'
    CONFIG_FILE_NAME = 'bgpd.conf'

    def __init__(self, host_dir, conf, image='bgperf/openbgp87'):
        super(OpenBGPTarget87, self).__init__(host_dir, conf, image=image)
